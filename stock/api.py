    ...
    Returns:
        DataFrame:
            ...
            >> get_exchange_list()
                 ...
                 통화명     매매기준율  미화환산율
        0         미국USD  1,178.70  1.000
        1       유럽연합EUR  1,335.76  1.133
        2   일본JPY(100엔)  1,038.00  0.881
        3         중국CNY    185.06  0.157
        4         홍콩HKD    151.19  0.128
        ....

            >> get_exchange_list("USD")

                 통화명     매매기준율  미화환산율
        0        미국USD  1,178.30  1.000
...
def get_ipo_schedule(date = None, manager = None):
    """환율 조회
    Args:
        date (datetime): 날짜 ("2021-21-01" ...)
        manager (str): 주간사 ("대신증권", "IBK투자증권" ...)
    Returns:
        DataFrame:

            >> get_ipo_schedule()

                           종목명        시작일        종료일   확정공모가            희망공모가      청약경쟁률                                    주간사
            0             래몽래인 2021-12-20 2021-12-21       -    11,500~13,000                                           IBK투자증권
            1           애드바이오텍 2021-12-20 2021-12-21       -      7,000~8,000                                              대신증권
            2        하나금융스팩20호 2021-12-16 2021-12-17       -      2,000~2,000                                            하나금융투자
            3          NH스팩22호 2021-12-16 2021-12-17       -      2,000~2,000                                            NH투자증권
            4              오토앤 2021-12-14 2021-12-15       -      4,200~4,800                                      미래에셋증권,현대차증권
            5           하이스팩7호 2021-12-09 2021-12-10       -      2,000~2,000                                            하이투자증권
        ....

            >> get_ipo_schedule("2021-12-21", "대신증권")

                   종목명        시작일        종료일 확정공모가        희망공모가 청약경쟁률   주간사
            1   애드바이오텍 2021-12-20 2021-12-21     -  7,000~8,000        대신증권
            
            >> get_ipo_schedule("2021-12-21")

                   종목명        시작일        종료일 확정공모가          희망공모가 청약경쟁률      주간사
            0     래몽래인 2021-12-20 2021-12-21     -  11,500~13,000        IBK투자증권
            1   애드바이오텍 2021-12-20 2021-12-21     -    7,000~8,000           대신증권
            
            >> get_ipo_schedule(manager = "대신증권")

                        종목명        시작일        종료일   확정공모가          희망공모가      청약경쟁률                      주간사
            1        애드바이오텍 2021-12-20 2021-12-21       -    7,000~8,000                                대신증권
            10   대신밸런스스팩11호 2021-11-22 2021-11-23   2,000    2,000~2,000   150.83:1                     대신증권
            15           트윔 2021-11-08 2021-11-09  22,000  17,800~20,400  2266.84:1                     대신증권
            16   NH올원리츠(유가) 2021-11-03 2021-11-05   5,000    5,000~5,000   453.48:1       한국투자증권,NH투자증권,대신증권
            21         지니너스 2021-10-27 2021-10-28  20,000  24,700~32,200    162.5:1              대신증권,NH투자증권
            22    카카오페이(유가) 2021-10-25 2021-10-26  90,000  60,000~90,000     29.6:1  삼성증권,대신증권,한국투자증권,신한금융투자
            24           엔켐 2021-10-21 2021-10-22  42,000  30,000~35,000  1275.69:1              대신증권,신한금융투자
        
    """
    df = com.get_ipo_schedule()
    df['시작일'] = pd.to_datetime(df['시작일'], format='%Y-%m-%d')
    df['종료일'] = pd.to_datetime(df['종료일'], format='%Y-%m-%d')
    if date != None:
        date = datetime.datetime.strptime(date, "%Y-%m-%d")
        mask = (df['시작일'] <= date) & (df['종료일'] >= date)
        df = df.loc[mask]
    if manager != None:
